...
- WebDAV uses a virtual `/webdav/` mount configured per Agent (`enabled`, `directories[]` with `path`, `permission`, `description`). Preserve directory hierarchy and compile native DeepAgent FilesystemPermission rules with child paths first and unselected paths denied. Global sync has no permission rules. When preparing upgrades, reconcile existing config explicitly; do not add symlinks or automatic migrations. Check native file permissions and HITL approval/resume together. WebDAV writes use interrupt rules; private/local shared text writes stay automatic. Remove retired search_context/write_context IDs from existing configs and run snapshots with a backed-up one-time operation.
...
- Explicit file approval leases last 600 seconds and apply only to one exact WebDAV file within one session and Agent. Keep ordinary approve one-shot. Verify persisted expiry without renewal on restart, per-call checks for main/subagents, unchanged filesystem deny rules, and file/Agent/session isolation; mixed pending targets must not receive a single-file grant.

- Frontend development, tests and builds require Node.js 20+ for the Markdown editor dependencies. Check the runtime before npm commands; production still serves committed web/dist and does not need a frontend dependency install.
