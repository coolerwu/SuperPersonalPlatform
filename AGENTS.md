...
- Web Chat 支持上传图片作为多模态输入：页面通过 `POST /api/chat/messages` 的 `attachments[]` 提交 `data_url`，后端在创建 run 前校验数量（最多 6 张）、大小（单张 20MB）和字节头（PNG/JPEG/GIF/WebP），再落到 `workspace/sessions/{session_id}/attachments/{message_seq}/`；纯图片消息允许正文为空。历史图片只通过 `GET /api/chat/sessions/{session_id}/attachments` 读取已记录的附件路径，Provider 不支持图片时沿用 `image_attachments_textified` 文本降级。
