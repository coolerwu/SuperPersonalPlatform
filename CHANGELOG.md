...
- Chat 输入区的附件动作收到左侧“+”菜单：发送按钮回到最右侧，点“+”展开当前只有“图片”一项的
  动作菜单（`Esc`/点击外部关闭、运行中或满 6 张时禁用，群聊不渲染），为后续文件、截图等入口留位置；
  粘贴图片改为由输入区拦截成待发送图片，不再有机会把剪贴板图片插进 Markdown 草稿。需求见
  `docs/requirements/F8-输入区动作入口.md`。
