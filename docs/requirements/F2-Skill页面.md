...
3. 技能条目显示 frontmatter 的 `name`、`description` 摘要、修改时间，以及“不生效”“契约”标记。
4. 右侧编辑 `SKILL.md` 文本，支持保存与删除；删除整个技能目录前二次确认。
5. 新建技能时收集 Agent、技能 ID、描述，生成带 frontmatter 的模板并创建文件。
6. 保存前校验 frontmatter，不合法即阻止保存并给出具体原因。
...
- name 与目录名不一致、缺 name、缺 description、description 超过 1024 字符、frontmatter 未闭合时保存被拒绝。
- 保存合法内容后文件内容更新，页面提示“下一次 Agent 运行时生效”。
- 删除确认后技能目录消失，列表刷新；取消确认则不删除。
- 包含用户契约区块的技能在编辑器上显示提示；页面本身允许用户修改该区块。
- 技能目录缺失、`SKILL.md` 缺失、列表为空都不会让页面报错崩溃。
