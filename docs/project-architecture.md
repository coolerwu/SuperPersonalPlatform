...
- The frontend contains a login page, a full-height console app shell, a home overview, an Agent page, a self-development workbench page, an iframe-based Hermes UI proxy page, a terminal page, and a system page split into config, logs, and update tabs. Menu pages fill the right-side workspace directly without a repeated global page title/header or floating outer card; individual tools such as chat, logs, terminal output, and configuration groups keep their own local panels. Workbench-style pages use 100% available width and viewport-height sizing with `100dvh` fallbacks for mobile browser chrome. The Agent page has a chat tab and a workspace configuration tab for models, platform tools, and Agent personalities, and its chat/config workspace scrolls content inside the panel instead of stretching the whole page. The self-development workbench uses a desktop three-column layout and stacks task list, main workbench, and detail sidebar into clearly separated single-column sections at small widths. A dedicated small-screen breakpoint (<=430px, including iPhone 14 width) further optimizes sidebar navigation as compact horizontal chips with active-item auto-centering, safe-area padding, chat composer stacking, tighter self-development spacing, and panel heights for one-hand mobile use.
