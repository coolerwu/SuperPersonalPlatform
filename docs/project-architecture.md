...
- 运行时不注入 Skill 自进化 middleware：Agent 不会自动创建或改写 `/skills/`，也没有 `/improvements/` 审计目录；skill 只在用户明确要求时创建或更新，用户在对话里要求时 Agent 用原生文件工具写入 `/skills/{skill_id}/SKILL.md`，也可以直接通过平台文件入口维护。Skill 可用 `<!-- BEGIN USER CONTRACT -->` / `<!-- END USER CONTRACT -->` 标出硬约束，`AgentFilesystemBackend` 会在 write/edit/upload/delete 入口逐字保护该区块及包含它的 Skill 文件/目录，正式契约只能由用户通过平台文件编辑入口修改。只有 `/skills/{skill_id}/SKILL.md` 会在下一次 Agent 执行开始时作为 skill metadata 被扫描。
