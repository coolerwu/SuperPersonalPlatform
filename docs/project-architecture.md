...
Chat 页面的图片上传入口位于输入区：附件按钮选择图片、直接粘贴剪贴板图片、把图片文件拖进聊天区都会走同一套校验（`chatAttachments.js` 先按扩展名/声明 MIME 过滤并读成 data URL，最多 6 张、单张不超过 20MB），待发送图片以可单个移除的缩略图挂在输入框上方；只发图片不写文字也能发送，发送失败时文字和图片一起恢复到输入区。用户消息气泡内按 `attachments` 渲染图片缩略图，点击在新标签页打开原图，历史消息刷新后通过 `GET /api/chat/sessions/{session_id}/attachments` 重新取图。技能审批沿用 Run 的审批卡片：Agent 改写 `/skills/**` 时会话进入 `waiting_approval`，页面上出现审批面板，批准或拒绝后才继续执行。
