...
- `WebDAVPathPolicy` 将目录配置编译为 DeepAgent 原生 `FilesystemPermission`，按路径深度降序让子目录覆盖父目录，支持父只读/子读写及父读写/子只读，用户无需排序。规则以 `/webdav/**` 默认拒绝收尾，不影响私有工作区；路径中的 glob 特殊字符按字面转义。`create_deep_agent(permissions=...)` 传给主 Agent，通用子 Agent 继承同一规则。Context 和后端直接访问复用原生 `_check_fs_permission`（私有 API 接口由回归测试约束），不另造匹配语义。未选择的祖先只允许目录导航，不能读取同层文件、无关兄弟或被替换成文件的祖先。
...
- `search_context` 合并 `/files/...` 与当前 Agent 映射范围内的 `/webdav/...`；未启用映射时不可通过 Context 工具访问 WebDAV。工具路径保留全局同步根下的原目录层级，recent_documents 在当前映射内筛选后按修改时间排序。共享索引不记录 Agent 权限；原生文件工具支持列举、读取、搜索及文本创建、编辑，写入先更新远端再更新缓存与索引。WebDAV 暂不支持原生删除及二进制上传。独立服务实例通过共享 `context/webdav/sync.lock` 文件锁协调同步和写入，编辑在锁内读取远端最新内容；认证和服务错误不当作文件缺失。
