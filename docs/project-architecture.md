...
- Context 可配置坚果云 WebDAV readable roots 和 writable roots；后台按 `context.webdav_sync.interval_seconds` 增量同步 readable roots 到 `workspace/context/state/webdav_cache/`，`search_context` 合并本地 `/files/...` 与 WebDAV `/webdav/{root_id}/...` 缓存检索。`write_context` 只能写 `writable=true` 且 `protected=false` 的 WebDAV root；`protected=true` root 可读可检索但不可写、覆盖或删除。
