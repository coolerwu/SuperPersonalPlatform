...
- `write_context(type, absolute_path, content, mode)` 支持 `type="knowledge"` 和 `append/overwrite/create`，本地路径使用 `/files/...`，WebDAV 路径使用当前 Agent 映射内的 `/webdav/...`。授权、范围和写回逻辑与原生文件工具共用；系统工具注册表将其标记为需审批。错误返回 `ok=false` 观察，不让整个 run 失败；远端成功、缓存更新失败时 diagnostics.remote_written=true。该工具不负责 Agent 私有记忆。
...
- `agents.definitions[].webdav` 配置包含 `enabled=false`、`path="/"`、`permission="write"`（可选 `read`）和 `description=""`。path 相对于全局同步根目录，每个 Agent 本轮一个映射；write 包含读取。运行快照保存该配置。`CompositeBackend` 将 `/webdav/` 路由至 WebDAVFilesystemBackend，默认后端仍为私有 AgentFilesystemBackend；不复制缓存、不使用软链接，`WebDAVPathPolicy` 在统一工作区权限模块中负责映射和授权。
- `search_context` 合并 `/files/...` 与当前 Agent 映射范围内的 `/webdav/...`；未启用映射时不可通过 Context 工具访问 WebDAV。映射前缀不出现在工具路径中，recent_documents 在当前映射内筛选后按修改时间排序。共享索引不记录 Agent 权限；原生文件工具支持列举、读取、搜索及文本创建、编辑，写入先更新远端再更新缓存与索引。WebDAV 暂不支持原生删除及二进制上传。独立服务实例通过共享 `context/webdav/sync.lock` 文件锁协调同步和写入，编辑在锁内读取远端最新内容；认证和服务错误不当作文件缺失。
...
- DeepAgent 原生 filesystem 使用受限的 `AgentFilesystemBackend(root_dir=workspace/agents/{agent_id}/workspace, virtual_mode=True)`。Agent 看到的 `/` 就是自己的私有目录；私有文件后端只允许修改 `scratch/`、`notes/`、`artifacts/`、`skills/`、`memories/`、`improvements/` 和 `meditations/`，并保护这些固定顶层目录本身不被删除。`browser/` 只由浏览器服务访问，Agent 文件工具的同步/异步读取、列举、搜索、下载和修改都不能触及其内容；不允许通过符号链接访问。启用的 `/webdav/` 由独立映射后端访问共享缓存，其只读/读写规则与 Context 工具一致；不能删除挂载根目录。Agent 不应创建第二层 `/workspace/`，不能访问其它未声明顶层目录；越界写入、编辑、删除和上传返回带允许目录列表的 permission 诊断，作为可恢复工具观察交给 Agent 改用正确路径，不让整个 run 失败。历史目录由部署时受控的一次性文件操作整理，没有运行时兼容路径或自动迁移。Agent 不能直接访问 `workspace/config.yaml`、`workspace/context`、`workspace/runs`、`workspace/sessions`、其它 Agent 目录或项目源码。旧的 run 前加载 `files` state、run 后同步回磁盘机制及辅助函数已删除。
