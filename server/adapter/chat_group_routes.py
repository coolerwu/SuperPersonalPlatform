...
def create_chat_group_router(container):
    ...
    async def send(group_id: str, payload: GroupMessageRequest):
        return await call(service.send(group_id, payload.content, payload.mentions, payload.client_message_id, reply_to_message_id=payload.reply_to_message_id, reply_excerpt=payload.reply_excerpt))

    @router.post("/{group_id}/collaborations")
    async def collaborate(group_id: str, payload: GroupMessageRequest):
        return await call(service.send(group_id, payload.content, payload.mentions, payload.client_message_id, automatic=True, reply_to_message_id=payload.reply_to_message_id, reply_excerpt=payload.reply_excerpt))
